 //src\app\statistics\usersStatistics\page.tsx
 'use client'
 ...
 export default function UserStatisticsPage() {
   ...
   const { accessRequests, loading: accessRequestsLoading, error: accessRequestsError } = useAccessRequests();
 
   const stats = useMemo(() => {
     if (!appUsers.length || !sponsorships.length || !reclamations.length || !accessRequests.length) return null;
    ...
    const accessRequestTrend = accessRequests.reduce((acc, request) => {
     ...
     return {
       ...
       accessRequestTrend: Object.entries(accessRequestTrend).map(([date, count]) => ({ date, count })),
      };
    }, [appUsers, sponsorships, reclamations, accessRequests]);
 
    if (usersLoading || sponsorshipsLoading || reclamationsLoading || accessRequestsLoading) return <LoadingSpinner />;
    if (usersError || sponsorshipsError || reclamationsError || accessRequestsError) 
      return <div>Erreur: {usersError || sponsorshipsError || reclamationsError || accessRequestsError}</div>;
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
       ...
       <Card title="Tendance des demandes d'inscription">
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={stats.accessRequestTrend}>
            <XAxis dataKey="date" />
            <YAxis />
            <Tooltip />
            <Legend />
            <Line type="monotone" dataKey="count" name="Nombre de demandes" stroke="#82ca9d" />
          </LineChart>
        </ResponsiveContainer>
      </Card>
    </div>
