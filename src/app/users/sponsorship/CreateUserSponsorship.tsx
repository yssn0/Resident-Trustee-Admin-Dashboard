// src\app\users\sponsorship\CreateUserSponsorship.tsx
import { useState } from 'react';
import { Sponsorship } from '@/types/sponsorship';
import LoadingSpinner from '@/components/ui/LoadingSpinner';

interface CreateUserFromSponsorshipProps {
...
export default function CreateUserFromSponsorship({ sponsorship, onClose }: CreateUserFromSponsorshipProps) {
  const [isCreating, setIsCreating] = useState(false);
  const [errors, setErrors] = useState<FormErrors>({});
  const [message, setMessage] = useState<string | null>(null);
  const [newUser, setNewUser] = useState({
    name: sponsorship.name,
    surname: sponsorship.surname,
    email: sponsorship.email,
    phoneNumber: sponsorship.phoneNumber,
    userType: 'user',
    password: '', 
  });
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      // If user creation is successful, delete the sponsorship
      const deleteSponsorshipResponse = await fetch('/api/delete_sponsorship', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ _id: sponsorship._id }),
      });
      ...
      setMessage('Utilisateur créé avec succès et parrainage supprimé');
      setTimeout(() => {
        onClose();
      }, 2000);
    } catch (error) {
      console.error('Error creating user or deleting sponsorship:', error);
